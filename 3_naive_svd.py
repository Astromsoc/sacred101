...
def main():
    ...
    # show several similar movies
    mids_first5 = list([mid for ud in train_umr.values() for mid in ud.keys()])[:5]
    for mid in mids_first5:
        neighbors = new_svd.m_neighbors[mid][:20]
        print(f"\n\n20 most similar movies to {mid}:")
        for mmid, simi in neighbors:
            print(f"  | mid = {mmid:<40} | similarity = {simi}")
